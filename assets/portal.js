  ...
  function openProgram(id) {
    ...
    api('/programs/' + id).then(function (r) {
      ...
      r.days.forEach(function (d) {
        var card = h('div', { class: 'card' }, [
          h('h2', { text: 'Week ' + d.week_index + ' · Day ' + d.day_index + (d.block_label ? '  (' + d.block_label + ')' : '') }),
          d.title ? h('div', { class: 'muted', text: d.title }) : null
        ]);
        (d.exercises || []).forEach(function (ex) {
          card.appendChild(h('div', { class: 'ex' }, [
            h('b', { text: ex.entry_type === 'text' ? (ex.block_text || ex.exercise_name) : ex.exercise_name }),
            ex.entry_type !== 'text' ? h('div', { class: 'rx', text: rxLine(ex).head.replace(ex.exercise_name + '  ·  ', '') + (ex.circuit_group ? '  · circuit ' + ex.circuit_group : '') + (parseInt(ex.is_optional, 10) ? ' · optional' : '') }) : null
          ]));
        });
