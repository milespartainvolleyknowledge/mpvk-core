...
class MPVK_Schema {
	...
	public static function install(): void {
		...
		// ---- Exercise library (v5: live module — category/equipment/level for the builder) ----
		...
		// ---- Templates / programs (v5: periodization fields — weeks, blocks, progression) ----
		// block labels and load notation are deliberately FREEFORM so any methodology
		// (Brent's included) fits without schema change.
		$sql[] = "CREATE TABLE {$t('templates')} (
			...
			athlete_level VARCHAR(20) NULL,
			weeks INT UNSIGNED NOT NULL DEFAULT 1,
			days_per_week INT UNSIGNED NOT NULL DEFAULT 3,
		...
		$sql[] = "CREATE TABLE {$t('template_days')} (
			...
			org_id BIGINT UNSIGNED NOT NULL,
			week_index INT UNSIGNED NOT NULL DEFAULT 1,
			day_index INT UNSIGNED NOT NULL,
			block_label VARCHAR(60) NULL,
		...
		$sql[] = "CREATE TABLE {$t('template_day_exercises')} (
			...
			progression_rule VARCHAR(120) NULL,
			entry_type VARCHAR(12) NOT NULL DEFAULT 'sets',
			rep_unit VARCHAR(12) NOT NULL DEFAULT 'reps',
			load_mode VARCHAR(12) NOT NULL DEFAULT 'weight',
			is_optional TINYINT(1) NOT NULL DEFAULT 0,
			circuit_group VARCHAR(20) NULL,
		...
		// ---- Pre-workout readiness check-ins (v7): joint/tendon health sliders ----
		// scores is JSON { area: 0-10 } (10 = 100% healthy, 0 = significant pain).
		$sql[] = "CREATE TABLE {$t('workout_checkins')} (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			workout_id BIGINT UNSIGNED NOT NULL,
			org_id BIGINT UNSIGNED NOT NULL,
			client_user_id BIGINT UNSIGNED NOT NULL,
			...
			KEY client_time (client_user_id, created_at)
		) $charset;";
		...
		// ---- Passkeys (WebAuthn credentials; v4) ----
		// cred_id is base64url of the raw credential id (unique per authenticator+site).
		// public_key is PEM (SPKI). counter enables clone detection per WebAuthn spec.
		$sql[] = "CREATE TABLE {$t('passkeys')} (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			user_id BIGINT UNSIGNED NOT NULL,
			cred_id VARCHAR(700) NOT NULL,
			cred_id_hash CHAR(64) NOT NULL,
			public_key TEXT NOT NULL,
			alg SMALLINT NOT NULL DEFAULT -7,
			counter BIGINT UNSIGNED NOT NULL DEFAULT 0,
			label VARCHAR(120) NULL,
			created_at DATETIME NOT NULL,
			last_used_at DATETIME NULL,
			PRIMARY KEY  (id),
			UNIQUE KEY cred_id_hash (cred_id_hash),
			KEY user_id (user_id)
		) $charset;";

		// ---- AI draft replies (v4; the approval-queue foundation) ----
		// One draft per inbound client message that needs a coach reply. status:
		// pending → generating; ready → shown to coach; used/dismissed/failed → terminal.
		$sql[] = "CREATE TABLE {$t('ai_drafts')} (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			org_id BIGINT UNSIGNED NOT NULL,
			thread_key VARCHAR(80) NOT NULL,
			message_id BIGINT UNSIGNED NOT NULL,
			...
			KEY thread_status (thread_key, status)
		) $charset;";
