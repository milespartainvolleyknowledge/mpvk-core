...
class MPVK_Seed {
	...
	public static function demo( int $coach_user_id = 0 ): array {
		...
		// Coach user (org tier). A strong random password is generated and surfaced once
		// via transient ONLY when the account is first created — re-seeding NEVER changes
		// the password of an existing demo account, so a login you're using stays valid.
		// (Forgot it? Reset from Users like any other account.)
		if ( ! $coach_user_id ) {
			$existing = username_exists( 'coach_demo' );
			if ( $existing ) {
				$coach_user_id = (int) $existing; // keep existing password untouched
			} else {
		...
		foreach ( array( 'ava', 'ben', 'cody', 'dana' ) as $i => $name ) {
			...
			if ( $existing ) {
				$uid = (int) $existing; // keep existing password untouched
			} else {
				$pass = wp_generate_password( 16, false );
