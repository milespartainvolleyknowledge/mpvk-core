...
class MPVK_Seed {
	...
	public static function demo( int $coach_user_id = 0 ): array {
		...
		// no fixed/known credentials ever exist. On re-seed, existing demo accounts get a
		// FRESH password too, so "Seed / refresh demo data" always hands back working logins.
		if ( ! $coach_user_id ) {
			$existing = username_exists( 'coach_demo' );
			$pass     = wp_generate_password( 16, false );
			if ( $existing ) {
				$coach_user_id = (int) $existing;
				wp_set_password( $pass, $coach_user_id ); // reset existing demo password
		...
		foreach ( array( 'ava', 'ben', 'cody', 'dana' ) as $i => $name ) {
			...
			if ( $existing ) {
				$uid = (int) $existing;
				wp_set_password( $pass, $uid ); // reset existing demo password
				$creds[ $login ] = $pass;
			} else {
