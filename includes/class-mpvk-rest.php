...
class MPVK_REST {
	...
	public static function get_workout( WP_REST_Request $req ): array|WP_Error {
		...
		return array(
			'workout'       => (array) $row,
			'exercises'     => $exercises,
			'checkin'       => MPVK_Programs::checkin_for( (int) $row->id ),
			'checkin_areas' => MPVK_Programs::checkin_areas(),
		);
	}
	...
	public static function send_message( WP_REST_Request $req ): array|WP_Error {
		...
		// Coach used an AI draft for this reply → record the training signal.
		$draft_id = (int) $req->get_param( 'draft_id' );
		if ( $draft_id && $actor !== $client ) {
			MPVK_AI::mark_used( $draft_id, $body );
		}

		// Let listeners react to the new message (AI draft queue hooks here).
		do_action( 'mpvk_message_sent', $msg_id, array(
			'org_id'     => $org,
			'thread_key' => $key,
			'sender'     => $actor,
			'recipient'  => $recipient,
