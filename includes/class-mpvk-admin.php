...
class MPVK_Admin {
	...
	public static function init(): void {
		...
		add_action( 'admin_post_mpvk_checkin_save', array( __CLASS__, 'handle_checkin_save' ) );
	}
	...
	public static function render(): void {
		...
		<div class="wrap">
			...
			<h2 style="margin-top:2em">Pre-workout readiness check-in</h2>
			<p class="description" style="max-width:640px">Athletes rate these areas (0–10, joint/tendon health) before each workout; scores show on the session and feed the AI analysis. Comma-separated — reorder or change to fit what you track.</p>
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="max-width:520px">
				<?php wp_nonce_field( 'mpvk_checkin' ); ?>
				<input type="hidden" name="action" value="mpvk_checkin_save">
				<input type="text" name="mpvk_checkin_areas" style="width:100%" value="<?php echo esc_attr( implode( ', ', MPVK_Programs::checkin_areas() ) ); ?>">
				<p><button class="button">Save check-in areas</button></p>
			</form>
	...
	public static function handle_checkin_save(): void {
		if ( ! current_user_can( 'manage_options' ) || ! check_admin_referer( 'mpvk_checkin' ) ) {
			wp_die( 'Not allowed.' );
		}
		$raw   = isset( $_POST['mpvk_checkin_areas'] ) ? (string) wp_unslash( $_POST['mpvk_checkin_areas'] ) : ''; // phpcs:ignore
		$areas = array_values( array_filter( array_map( 'trim', explode( ',', $raw ) ) ) );
		$areas = array_map( 'sanitize_text_field', array_slice( $areas, 0, 10 ) );
		update_option( 'mpvk_checkin_areas', $areas ? wp_json_encode( $areas ) : '' );
		wp_safe_redirect( admin_url( 'admin.php?page=mpvk' ) );
		exit;
	}
